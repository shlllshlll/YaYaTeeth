...
def _convert_dataset(dataset_split):
    ...
    label_jpg_reader = build_data.ImageReader('jpeg', channels=1)
    label_png_reader = build_data.ImageReader('png', channels=1)
        ...
        with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
            ...
            for i in range(start_idx, end_idx):
                ...
                if not Path(image_filename).exists():
                    print(f"{image_filename} not exists.")
                    continue
                ...
                seg_filename = os.path.join(
                    FLAGS.semantic_segmentation_folder,
                    filenames[i] + '.jpg')
                label_reader = label_jpg_reader
                if not Path(seg_filename).exists():
                    seg_filename = os.path.join(
                        FLAGS.semantic_segmentation_folder,
                        filenames[i] + '.png')
                    label_reader = label_png_reader
